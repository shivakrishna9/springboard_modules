(function ($) {
  Drupal.behaviors.improved_multi_select = {
    attach: function(context, settings) {
      if (settings.improved_multi_select && settings.improved_multi_select.selectors) {
        ...
        for (var key in options.selectors) {
          ...
          $(selector, context).once('improvedselect', function() {
            ...
            $select.parent().append(
              '<div class="improvedselect" sid="' + $select.attr('id') + '" id="improvedselect-' + $select.attr('id') + '">' +
                '<div class="improvedselect-text-wrapper">' +
                  '<input type="text" class="improvedselect_filter" sid="' + $select.attr('id') + '" />' +
                '</div>' +
            ...
            $select.find('option, optgroup').each(function() {
              var $opt = $(this),
                group = '';
              if (improvedselectIsOptgroup($opt)) {
        ...
        $('.improvedselect_filter', context).once('improvedselect', function() {
  ...
  function improvedselectIsOptgroup($opt) {
      var tagName = '';

      // jQuery 1.6+ uses .prop() for tagName
      // Earlier versions use .attr()
      try {
          tagName = $opt.prop('tagName');
      }
      catch(err) {
          tagName = $opt.attr('tagName');
      }
